...
<VSheet class="mx-auto" width="350">
  <VForm ref="formRef" @submit.prevent="onSubmit">
    ...
    <h4 class="text-h5">Horario de consulta</h4>
    <VSelect
      v-model="diasConsulta"
      :items="items.dias"
      label="Dias de consulta"
      multiple
      clearable
...
const cursosItems = ['Lunes', 'Martes', 'Miércoles', 'Jueves']; 
...
const onSubmit = async () =>{
  ...
  if(valid ){
    const texto = nombre.value + " " + cursos.value + " " + diasConsulta.value + " " + horaInicio.value + " " + horaFin.value + " " + cantCitas.value
    Alerta.showExitoSimple()
  }
}
